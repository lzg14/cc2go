...
class TestConvertMessages(unittest.TestCase):
    ...
    def test_complete_tool_sequence_preserved(self) -> None:
        """tool_calls + 完整 tool 响应 → 不做修改"""
        messages = [
    ...
    def test_partial_tool_results_fills_only_missing(self) -> None:
        ...
        messages = [
            ...
            {"role": "tool", "content": "bash ok", "tool_call_id": "tc_a"},
            {"role": "user", "content": "continue"},
        ]
        result = convert_anthropic_messages_to_openai(messages)
        # assistant + tool(tc_a) + tool(tc_b empty) + user
        self.assertEqual(len(result), 4)
        self.assertEqual(result[1]["tool_call_id"], "tc_a")
        self.assertEqual(result[1]["content"], "bash ok")
        self.assertEqual(result[2]["tool_call_id"], "tc_b")
        self.assertEqual(result[2]["content"], "")
        self.assertEqual(result[3]["role"], "user")
    ...
    def test_multiple_tool_use_sequences_both_incomplete(self) -> None:
        ...
        messages = [
            ...
            {"role": "user", "content": "oops wrong context"},
            {"role": "assistant", "content": None, "tool_calls": [
                {"id": "tc_2", "type": "function", "function": {"name": "read", "arguments": "{}"}}
            ]},
        ]
        result = convert_anthropic_messages_to_openai(messages)
